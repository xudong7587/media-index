...
export function OpenListTaskMonitor({ tasks, compact = false, emptyText = "当前通路没有复制任务" }: {
  ...
  return <div className={`openlist-task-monitor ${compact ? "compact" : ""}`}>
    {tasks.map((task) => <article className={`openlist-task-row ${task.state}`} key={task.id || `${task.name}-${task.start_time}`}>
      ...
      <div className="openlist-task-body">
        ...
        <small>{task.error || (task.state === "done" ? "复制完成" : task.status || "正在复制")}</small>
...
export function OpenListTaskPanel({ limit = 20 }: { limit?: number }) {
  ...
  const [message, setMessage] = useState("正在读取复制队列");
  useEffect(() => {
    let active = true;
    let loading = false;
    const refresh = async () => {
      ...
      try {
        ...
        if (active) { setTasks([]); setMessage(error instanceof Error ? error.message : "复制队列读取失败"); }
  ...
  return <section className="openlist-live-tasks"><header><div><h2>跨盘复制进度</h2><p>{message}</p></div><span>{tasks.filter((task) => task.state === "running").length} 个进行中</span></header><OpenListTaskMonitor tasks={tasks.slice(0, limit)} /></section>;
}
