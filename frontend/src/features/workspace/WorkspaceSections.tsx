...
type TransferRulesSection = "common" | "quark" | "p115";

export function TransferRulesPage({
  initialProvider = "common",
}: {
  initialProvider?: TransferRulesSection;
}) {
  const [config, setConfig] = useState<ConfigStatus | null>(null);
  const [provider, setProvider] = useState<TransferRulesSection>(initialProvider);
  const [form, setForm] = useState<Record<string, string>>({});
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState<Result>(null);
  const [directoryPicker, setDirectoryPicker] = useState<"root" | "staging" | null>(null);
    ...
    <section className="workspace-section">
      ...
      <div className="rules-layout">
        ...
        {(provider === "quark" || provider === "p115") && <SettingsSection title={`${provider === "p115" ? "115" : "夸克"} 保存位置`} body="这里只设置该网盘的根目录和任务暂存区；分类、命名和质量规则继承通用规则。">
          <SettingsInput label="保存根目录" name={rootName} value={rootValue} saved placeholder="/strm" onChange={update} showSavedValue action={<button type="button" className="ghost compact-action" onClick={() => setDirectoryPicker("root")}>选择目录</button>} />
          <SettingsInput label="任务暂存目录" name={stagingName} value={stagingValue} saved placeholder="/.media-index-staging" onChange={update} showSavedValue action={<button type="button" className="ghost compact-action" onClick={() => setDirectoryPicker("staging")}>选择目录</button>} />
      ...
      {directoryPicker && (provider === "quark" || provider === "p115") && <ProviderDirectoryPicker provider={provider} label={`${provider === "p115" ? "115" : "夸克"}${directoryPicker === "root" ? "保存根目录" : "任务暂存目录"}`} startPath={(directoryPicker === "root" ? rootValue : stagingValue) || "/"} onClose={() => setDirectoryPicker(null)} onSelect={(path) => { update(directoryPicker === "root" ? rootName : stagingName, path); setDirectoryPicker(null); }} />}
