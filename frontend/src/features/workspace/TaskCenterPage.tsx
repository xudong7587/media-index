...
export function TaskCenterPage() {
  ...
  async function retryOrganizer(job: TransferJob) { setBusy(true); try { const result = await api.retryCloudDownloadOrganizer(job.id); setMessage(result.message); await load(); setSelected(null); } finally { setBusy(false); } }
    ...
    <section className="workspace-section task-center-page">
      ...
      {tab === "running" && <>
        <div className="task-filter-row">
          {([['all','全部',jobs.length],['active','进行中',jobs.filter((job) => activeStates.has(job.status)).length],['review','待确认 / 需处理',jobs.filter((job) => job.status === 'needs_review').length],['failed','异常',jobs.filter((job) => ['failed','stopped'].includes(job.status)).length]] as const).map(([key,label,count]) => <button type="button" key={key} className={filter === key ? "active" : ""} onClick={() => setFilter(key)}>{label}<strong>{count}</strong></button>)}
        </div>
      ...
      {selected && createPortal(<div className="task-detail-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelected(null); }}>
        <article className="task-detail-panel" role="dialog" aria-modal="true" aria-label={`任务 ${selected.id} 详情`}>
          ...
          <div className="task-detail-status"><span className={`task-status ${selected.status}`}>{statusText(selected.status, selectedIsOrganizerReview)}</span><strong>{selected.stage || "等待执行"}</strong><p>{selected.message}</p></div>
          ...
          {selected.status === "needs_review" && (selectedIsOrganizerReview
            ? <section className="task-review-section"><h3>需要处理</h3><p>云下载整理不支持跳过安全校验强制确认。修正源目录名称、内容或目标命名冲突后，可以只重新核对这个任务记录的来源目录，不会扫描同级媒体，也不会再次转存分享文件。</p><button type="button" className="primary compact-action" disabled={busy} onClick={() => void retryOrganizer(selected)}><ArrowClockwise />重新核对当前目录</button></section>
