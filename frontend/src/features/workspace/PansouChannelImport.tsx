...
export function PansouChannelImport() {
  ...
  async function importChannels(channelIds: string[]) {
    ...
    try {
      ...
      setSelected([]);
      setMessage({ ok: true, text: `${result.message} 新频道已启用为搜索来源，未开启愿望单命中自动转存。` });
    } catch (error) {
  ...
  return <section className="pansou-channel-import">
    <div className="workspace-section-heading">
      <div>
        ...
        <p>直接读取 PanSou 当前配置的 Telegram 频道列表，规范化并对照 MediaIndex 已有频道；不通过资源搜索结果反推频道。</p>
      </div>
    ...
    <p className="settings-field-help">数据来自 PanSou <code>/api/health</code> 返回的 <code>channels</code> 配置；接口不提供频道列表时会明确提示无法自动导入。</p>
