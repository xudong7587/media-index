...
def _asset_values(asset: AssetInput) -> tuple[Any, ...]:
    ...
    inventory_root_path = _safe_inventory_root_path(asset.inventory_root_path)
    if int(asset.size) < 0:
...
def mark_missing_assets_unavailable(
    provider: str,
    *,
    parent_ids: set[str],
