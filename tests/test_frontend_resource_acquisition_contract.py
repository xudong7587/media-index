...
class FrontendResourceAcquisitionContractTests(unittest.TestCase):
    def setUp(self):
        ...
        self.channels = (ROOT / "frontend/src/features/tracking/ChannelWorkspace.tsx").read_text(encoding="utf-8")
        self.channel_styles = (ROOT / "frontend/src/features/tracking/channel-workspace.css").read_text(encoding="utf-8")
    ...
    def test_pansou_and_telegram_are_separate_tabs(self):
        ...
        self.assertIn('source === "pansou" ? <PansouSourceSettings /> : <ChannelWorkspace onOpenTelegramSettings=', self.sources)
    ...
    def test_channel_directory_is_bounded_searchable_and_filterable(self):
        ...
        self.assertIn("删除频道", self.channels)
        self.assertIn("全选当前筛选", self.channels)
        self.assertIn("从 PanSou 导入", self.channels)
    ...
    def test_global_tracking_settings_are_saved_separately(self):
        ...
        channel_body = self.channels.split("async function saveChannel", 1)[1].split("function toggleDeleteMode", 1)[0]
    ...
    def test_guide_is_a_full_workflow_manual_with_direct_routes(self):
        ...
        self.assertIn("chapterStepRoutes", self.guide)
        self.assertIn("onNavigate(route)", self.guide)
        self.assertIn("scrollToChapter(id)", self.guide)
