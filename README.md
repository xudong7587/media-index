...
1. 在“设置 → 全局设置”配置 TMDB，在“网盘工作台 → 资源获取”配置 PanSou；PanSou 反向关键词只过滤聚合搜索结果，不与 TG 规则共用。
2. 连接 115 或夸克，分别设置正式媒体库根、云下载根和允许整理的一级分类目录。
...
运行日志的“清除历史”只隐藏服务端终态记录，“停止运行”只终止活动任务，两者互不代替。MDC-NG 完成事件只会在恰好存在一个对应 Provider 的互动等待任务时关闭该任务；多个候选时保持原状态，避免猜测完成。
...
![Version](https://img.shields.io/badge/version-0.7.6-6d7cff?style=flat-square)
