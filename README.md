...
![Version](https://img.shields.io/badge/version-0.5.10-6d7cff?style=flat-square)
...
- **智能追更**：每次执行都读取目标网盘目录真实状态，只从最后已存集的下一集开始找资源；目录为空时才做初始全量处理。追更页顶部可直接开关自动巡检，巡检页可设置 TMDB 更新日后的追更时间、轮询周期、固定重试间隔和次数；每个网盘卡片可查看并调整该季实际保存路径。
- **按集转存、补集与同步**：发现页可按季展开检索到的剧集并选择部分转存；智能追更卡片支持同步所选、同步所有、补齐所选和补齐所有，也可粘贴已有夸克或 115 分享链接，只补所选集。
- **OpenList 按需同步较新资源**：当夸克有较新集时，通过 OpenList 只复制对应媒体目录中 115 缺失的文件；不会因开启自动同步而定时递归复制整个媒体库，相同同步任务运行中不会重复触发。115 → 夸克复制因 OpenList 115 跨存储读取问题暂时停用。
...
- TMDB 更新日期到达设定的追更时间后开始巡检；失败或资源尚未更新时按巡检页设置的间隔重试，达到次数后进入待处理状态。
...
同步会先刷新并读取两边目录，只复制缺失文件，并沿用各网盘设置的根目录、分类目录、媒体目录和 Season 路径；智能追更完成夸克转存后，会把媒体元数据带入同步流程，并兼容 OpenList 中已有的标题别名目录。如果 OpenList 列表暂时落后于网盘原生状态，会用对应网盘的真实文件名继续提交 OpenList 复制。相同目录已有同步任务在运行时不会重复提交。MediaIndex 通过 OpenList 的文件复制接口发起操作，OpenList 是否保留已完成的复制任务记录由其后台的任务持久化设置决定。由于 OpenList 的 115 → 夸克复制会在上传阶段失败，OpenList 手动同步入口和智能追更夸克侧的“同步所选/同步所有”暂时置灰；夸克 → 115 自动同步、追更执行和 115 侧选集同步保留。媒体库全量同步不由自动同步开关定时触发。
...
| `磁力链接` | 关联网盘为 115 时提交 115 离线下载，使用交互默认下载目录；裸链接不带媒体分类与季信息 |
| `/review` | 查看待确认任务 |
