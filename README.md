...
| 追更和通知 | 愿望单、智能追更、任务中心、运行日志；企业微信与 Telegram 可接收进度、待确认和入库通知。 |
...
- **通知与交互**：企业微信自建应用、企微机器人和 Telegram；支持图文通知、状态查看、待确认操作和分享链接转存。
...
`8000` 是管理面板，`8097` 是 STRM/302 播放入口。若 NAS 已占用端口，改左侧即可，例如 `38000:8000` 与 `38001:8097`；无需把宿主机端口写进 environment。`/strm` 必须挂载到 Emby 实际扫描的目录，并需具有写入 `.strm` 的权限。
...
3. 在 **转存和整理规则** 配置各网盘的根目录、分类路径、质量优先级、媒体文件夹/季/文件命名规则。
