...
- **原生 115 支持**：通过Cookie直接读取 115 分享、匹配文件、转存、改名、移动并确认目标目录，配合 MoviePilot 等其他服务实现自动化流程。
- **QAS 夸克转存**：复用 quark-auto-save 的夸克能力，自动实现视频真伪验证、改名、转存等操作。
- **多网盘并行**：发现页可同时验证夸克和 115；一个网盘失败可自动从另一个网盘复制同步（需openlist支持，目前仅支持从夸克复制到115）。
- **智能追更**：根据 TMDB 分集信息智能筛选和核验网盘资源，自动进行正确命名，命名可以自由设置规则。读取目标网盘目录真实状态，从已存集的下一集开始找资源；目录为空时做初始全量处理。资源尚未更新会静默重试，只有真实文件歧义才进入待确认。
