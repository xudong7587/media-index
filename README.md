...
![Version](https://img.shields.io/badge/version-0.7.32-6d7cff?style=flat-square)
...
- **资源入口彼此独立**：PanSou 聚合搜索与 TG 频道追踪分开配置；每个 TG 频道拥有独立的正向词、反向词、转存开关和云下载分类规则。
...
1. 在 **网盘工作台 → 网盘链接** 中启用夸克或 115，保存凭据并测试连接。115 扫码登录会保存所选终端模式，后续 STRM 播放使用同一模式获取直链；重新选择其他终端扫码后会自动更新绑定。

2. 在 **全局设置** 中配置 TMDB；需要站内搜索或频道追踪时，再到 **网盘工作台 → 资源获取** 分别配置 PanSou 与 TG。
