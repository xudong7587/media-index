...
- **可控 STRM 维护**：分别配置 115/夸克来源目录、可勾选的直接子目录、输出目录、全量扫描、增量扫描和 Cron 定期增量扫描；所有扫描均完整分页，不设 10,000 条总量上限。
...
- **通知与交互**：企业微信自建应用、企微机器人和 Telegram；媒体通知优先使用海报图文卡片与易读字段，支持状态查看、待确认操作和分享链接转存。
- **MDC-NG 联动**：MDC-NG 每次刮削成功可通过带密钥的 POST Webhook 通知 MediaIndex；连续 finished 事件会合并为一次只增不删的 STRM 增量同步。
...
- [MDC-NG](https://github.com/mdc-ng/mdc-ng)：可选；刮削成功后触发 MediaIndex 增量生成 STRM。
...
5. 在 **STRM 与 302** 分别设置网盘来源、STRM 输出目录和播放地址；将 `/strm` 与 Emby 媒体库目录对应。
6. 如需自动入库，在 **媒体服务器** 保存 Emby 地址、API Key；可选择媒体库封面样式、刷新与删除同步策略。
7. 最后在 **全局设置 → 通知和交互** 配置企业微信或 Telegram，并先发送测试通知。
8. 如使用 MDC-NG，在 **通知和交互 → Webhook** 生成密钥、选择网盘和来源目录，再按页面步骤把完整 Endpoint URL 填入 MDC-NG。
...
- 全量、手动增量、Cron 周期增量、115 生活监控和 MDC-NG 触发增量都会完整分页遍历当前范围，不设置 10,000 文件总量上限；内部仍以 500 条为一批写入本地数据库。勾选来源目录的直接子目录后，仅读取和维护这些目录；运行日志会显示当前扫描目录。
- 增量扫描永不执行缺失项清理，远端返回空清单也直接跳过清理。只有明确完成且非空的全量扫描才可推进清理：首次缺失进入 `pending_remove`，连续第二次完整全量扫描仍缺失才允许删除；计划删除超过当前范围 10%（小范围至少允许 1 条）或超过 50 条时触发熔断，本轮不删除任何 STRM。
- Emby 自动入库会根据当前 STRM 输出与媒体库关联刷新对应媒体库；通用设置不再展示历史的路径自动匹配与后备媒体库选项。
- Emby Webhook 使用 MediaIndex 设置页生成的完整回调 URL，并通过设置页的 Webhook 密钥校验。只对 MediaIndex 已登记且能唯一解析到 115 文件 ID 的 STRM 执行联动删除，不按名称猜测；当前只开放移入回收站，且必须显式开启自动执行。
