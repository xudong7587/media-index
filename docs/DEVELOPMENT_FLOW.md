...
1. 从当前 GitHub 最新正式 Release 开始一个任务分支；按 `docs/MODULE_BOUNDARIES.md` 声明一个 Primary Module，只读该模块、必要公开接口和相关测试。
2. 在本地修改；普通功能先在 `http://127.0.0.1:5173/` 验收。
3. 只运行与风险相称的测试。小修不构建、不跑全量；跨模块或高风险改动补回归测试。
4. 本地验收通过后停止，等待用户决定是否发布。

默认只修改 Primary Module。若必须改 Shared/Core 或第二个业务模块，先记录原因、受影响模块、兼容性和对应回归证据；不要借机整理无关代码。
