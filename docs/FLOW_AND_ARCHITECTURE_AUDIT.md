...
- 所有媒体优先搜索 TMDB `zh-CN` 简体中文规范名；TMDB 规范名为外文时，先搜索其已验证的中文别名；
- 每次 PanSou 查询都只包含片名，不携年份、季号、集号、播出日期、期数或画质词；
- 中文规范名/中文别名零结果时，才回退一次外文规范名或 TMDB 英文译名；不轮询繁体名、未验证别名或各种年份、季集排列；
- 电视剧/综艺候选只按名称和明确季标记筛选，不使用首播年、季播出年或上传年；第一季等价识别 `第一季`、`第1季`、`S1`、`S01`、`Season1` 和 `Season 1`；
- 电影仍可用年份消除同名歧义；集号和播出日期只用于剧集文件匹配。即使用户输入了季号或年份，也不得把这些条件拼入 PanSou 查询词。

中文别名只接受 TMDB 已验证的数据，并保持数量上限；不得恢复所有别名与季集条件的全排列搜索。
...
| 规范中文查询策略 | 已完成 | 仅简体中文标准名；零结果才用 TMDB 英文译名 |
