...
- 增量任务只创建、替换和校验映射，不能推进缺失项清理。MDC-NG Webhook、Cron、115 生活监控和转存后处理都属于增量任务。
...
MDC-NG Webhook 使用独立密钥认证，外部 Body 不能指定 Provider 或扫描路径。连续 `finished` 事件先合并，再以服务端保存范围触发只增不删的 STRM 增量任务。
