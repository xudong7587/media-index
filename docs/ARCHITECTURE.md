...
Provider 为 QAS（夸克）和原生 115；MoviePilot 仅可作为 115 Cookie 的导入源或由用户
自行配置的 STRM 后处理器，不参与 MediaIndex 的核心转存事务。
...
- 夸克、115 和本地根路径由服务端分别配置，旧 `CLOUD_SAVE_PATH`/`CATEGORY_PATHS_JSON`
  继续作为夸克兼容回退值。
