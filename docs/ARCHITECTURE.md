...
云下载整理自 GitHub Release `v0.6.15` 正式提供。它的 Primary Module 是 `transfer`；`cloud`、`settings`、`strm`、`integrations` 是 Changed Modules，Shared/Core 中的调度器只负责触发与失败收口。Emby 仍通过既有转存后处理接缝刷新，不新增第二套媒体服务器流程。

```text
用户明确选择云下载根的直接子目录
  -> 以同名映射到正式媒体库根的直接子目录
  -> 扫描该范围下的一级媒体目录，并对直接媒体文件做保守标题分组
  -> 为每个整理单元记录清单指纹
  -> 等待目录在稳定窗口内不再变化
  ...
  -> 复用 post_transfer_pipeline 触发 STRM、Emby 与通知
```

该流程遵守以下不变量：

- 范围只能是已配置云下载根的直接子目录，目标固定为正式媒体库根下的同名子目录；浏览器不能提交任意目标绝对路径。云下载根本身的散落文件、未选择范围和范围之外的目录都不参与扫描。
- 每个选择范围下的一级媒体目录是整理单元；直接放在该范围的媒体文件，则按清洗后的标题、年份和剧集标记保守分组，无法提取文本身份时 fail closed。当前文件清单指纹必须在设定时间内保持不变；内容变化后重新等待。
...
- 整理完成复用 `post_transfer_pipeline`；STRM 仍只处理对应 Provider 已开启且已保存的来源/输出/直接子目录范围，Emby 刷新与通知仍受各自开关约束。该流程不能扩大后处理范围。
