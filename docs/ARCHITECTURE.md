...
云下载整理自 GitHub Release `v0.6.15` 正式提供。它的 Primary Module 是 `transfer`；`cloud`、`settings`、`strm`、`integrations` 是 Changed Modules。整理只由 MediaIndex 已知的精确转存完成事件触发，不注册周期扫描任务。Emby 仍通过既有转存后处理接缝刷新，不新增第二套媒体服务器流程。

```text
MediaIndex 的精确转存任务完成
  -> 从任务回执取得 Provider、目标路径、文件 ID 或文件名
  -> 验证对应 Provider 开关和已选云下载直接子目录
  -> 以同名映射到正式媒体库根的直接子目录
  -> 只读取该任务指向的媒体目录或精确文件
...
- 夸克和 115 分别有独立启动开关。范围只能是已配置云下载根的已选直接子目录，目标固定为正式媒体库根下的同名子目录；浏览器和外部请求都不能提交任意目标绝对路径。未开启 Provider、云下载根散落文件、未选择范围和范围之外的路径都不读取。
- 每次事件只能处理回执明确指向的一级媒体目录或精确文件；不会遍历同一分类中的其他媒体，也不会用全量或增量扫描兜底。任务回执不足以唯一定位媒体时 fail closed。
