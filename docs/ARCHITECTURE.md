...
- MediaIndex 自有整理完成后复用 `post_transfer_pipeline`；STRM 只接收本次已核验目标的路径与 Provider 文件 ID。MDC-NG Webhook 只作为完成通知，按服务端保存的来源根和已勾选子目录执行只增不删的增量扫描；外部请求不能覆盖 Provider 或扫描范围。Emby 刷新与通知仍受各自开关约束。
